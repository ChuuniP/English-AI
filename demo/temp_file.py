        ...
        with gr.Column(scale=4):
            ...
            with gr.Column(visible=False) as view_listen:
                ...
                with gr.Tabs():
                    ...
                    btn_start.click(
                        fn=lambda level: lm.process_start_practice(client, cefr_dict_sample, level, translator,
                                                                   lemmatizer),
                        inputs=[ceft_level],
                        outputs=[
                            setup_panel,
                    ...
                    with gr.Tab("Paragraph") as tab_listen:
                        with gr.Column() as select_topic_panel:
                            topic_dropdown = gr.Dropdown(label="Select topic to listen", filterable=True,
                                                         interactive=True, )
                    ...
                    with gr.Tab("Understanding"):
                        ...
                        submit_answer_btn.click(
                            ...
                            inputs=[current_index_state, answer_input, transcript_state, questions_state,
                                    answers_state],
                            outputs=[
                ...
                with gr.Tabs():
                    with gr.Tab("IPA"):
                        ...
                        with gr.Column(visible=False) as ipa_word_speaking_panel:
                            ...
                            play_word_speaking_btn = gr.Button("🔊", variant="secondary", size="sm",
                                                               elem_id="play-audio-btn")
                            with gr.Row():
                        ...
                        audio_recorder.change(
                            fn=lambda x, y, r, s: sm.handle_record_action(wav2vec2_processor, wav2vec2_model, device, x,
                                                                          y, r, s),
                            inputs=[
                                audio_recorder,
                                speaking_current_idx_state,
                    ...
                    with gr.Tab("1 minute") as tab_1minute:
                        # --- Panel 1: chỉ hiện lúc mới vào tab ---
                        with gr.Row(visible=True) as start_1minute_panel:
                        ...
                        # --- Panel 2: hiển thị topic (ẩn ban đầu) ---
                        with gr.Row(visible=False) as topic_1minute_panel:
                            topic_1minute_text = gr.Markdown(
                                "Topic: Say something",
                                elem_id="topic-1minute-card",
                            )

                        # --- Panel 3: nút đổi topic + nút ghi âm (ẩn ban đầu) ---
                        with gr.Row(visible=False) as btn_1minute_panel:
                        ...
                        # LƯU Ý: đã đổi data-target/elem_id từ "hidden-audio-recorder-sentence"
                        # sang "hidden-audio-recorder-1minute" để không đụng elem_id của tab Sentence.
                        gr.HTML("<style>#hidden-audio-recorder-1minute{display:none !important;}</style>")

                        audio_1minute_recorder = gr.File(
                            visible=True,
                            elem_id="hidden-audio-recorder-1minute",
                            type="filepath",
                        )

                        # --- Panel 4: kết quả (ẩn ban đầu) ---
                        with gr.Row(visible=False) as respond_1minute_panel:
                            with gr.Column():
                                with gr.Row():
                                    gr.Markdown("---")
            ...
            with gr.Column(visible=False) as view_read:
                with gr.Row():
                    ...
                    ui_change_inputs = [cached_reading_text, font_family_opt, font_size_opt, bg_color_opt,
                                        text_color_opt]

                    font_family_opt.change(fn=rm.render_html_reading_zone, inputs=ui_change_inputs,
                                           outputs=reading_display)
                    font_size_opt.change(fn=rm.render_html_reading_zone, inputs=ui_change_inputs,
                                         outputs=reading_display)
                    bg_color_opt.change(fn=rm.render_html_reading_zone, inputs=ui_change_inputs,
                                        outputs=reading_display)
                    text_color_opt.change(fn=rm.render_html_reading_zone, inputs=ui_change_inputs,
                                          outputs=reading_display)
